...
/**
 * Checks validity of given hue, saturation and brightness
 ...
 */
utils.validateHSB = function(hue, saturation, brightness, context) {
  if (typeof hue !== 'number' || hue < constants.HSBK_MINIMUM_HUE || hue > constants.HSBK_MAXIMUM_HUE) {
    throw new RangeError('LIFX ' + context + ' expects hue to be a number between ' +
  ...
  if (typeof saturation !== 'number' || saturation < constants.HSBK_MINIMUM_SATURATION || saturation > constants.HSBK_MAXIMUM_SATURATION) {
    throw new RangeError('LIFX ' + context + ' expects saturation to be a number between ' +
  ...
  if (typeof brightness !== 'number' || brightness < constants.HSBK_MINIMUM_BRIGHTNESS || brightness > constants.HSBK_MAXIMUM_BRIGHTNESS) {
    throw new RangeError('LIFX ' + context + ' expects brightness to be a number between ' +
      constants.HSBK_MINIMUM_BRIGHTNESS + ' and ' + constants.HSBK_MAXIMUM_BRIGHTNESS);
  }
};

/**
 * Checks validity of given zone index
 * @param {any} index Multi zone index to validate
 * @param {String} context validation context
 */
utils.validateZoneIndex = function(index, context) {
  if (typeof index !== 'number' || index < constants.ZONE_INDEX_MINIMUM_VALUE || index > constants.ZONE_INDEX_MAXIMUM_VALUE) {
    throw new RangeError('LIFX ' + context + ' expects zone to be a number between ' +

 ...
 * @param  {Number} obj.count between 0 and 256
 * @param  {Number} obj.index between 0 and 256
...
Packet.toBuffer = function(obj) {
  ...
  if (typeof obj.count !== 'number' && obj.count < 0 || obj.count > 256) {
    throw new RangeError('Invalid count value given for stateZone LIFX packet, must be a number between 0 and 256');
  }
  buf.writeUInt8(obj.count, offset);
  offset += 1;

  if (typeof obj.index !== 'number' && obj.index < 0 || obj.index > 256) {
    throw new RangeError('Invalid index value given for stateZone LIFX packet, must be a number between 0 and 256');
  }

...
Client.prototype.processDiscoveryPacket = function(err, msg, rinfo) {
  ...
  if (msg.service === 'udp' && msg.port === constants.LIFX_DEFAULT_PORT) {
    ...
    var discovering = this.discoveryTimer !== null;
    if (discovering) {
      var lightsDiscovered = _.keys(this.devices).length;
      var allDiscovered = lightsDiscovered >= this.lightAddresses.length;
      var allOnline = _.every(this.devices, function(device) {
        return device.status === 'on';
      });
      var labelsReceived = _.every(this.devices, function(device) {
        return _.isString(device.label);
      });

      if (allDiscovered && allOnline && labelsReceived && this.discoveryTimer) {
        console.log('All lights discovered, stopping discovery process');
        this.stopDiscovery();
      }
    }
  }

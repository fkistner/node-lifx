...
Light.prototype.color = function(hue, saturation, brightness, kelvin, duration, callback) {
  var validationContext = 'light color method';

  // Throw error on invalid values
  utils.validateHSB(hue, saturation, brightness, validationContext);
 ...
 * @param {Number} startIndex start zone index from 0 - 256
 * @param {Number} endIndex start zone index from 0 - 256
 * @param {Number} hue color hue from 0 - 360 (in °)
 * @param {Number} saturation color saturation from 0 - 100 (in %)
 * @param {Number} brightness color brightness from 0 - 100 (in %)
 * @param {Number} [kelvin=3500] color kelvin between 2500 and 9000
...
Light.prototype.colorZones = function(startIndex, endIndex, hue, saturation, brightness, kelvin, duration, apply, callback) {
  var validationContext = 'color zones method';

  // Throw error on invalid values
  utils.validateZoneIndex(startIndex, validationContext);
  utils.validateZoneIndex(endIndex, validationContext);
  utils.validateHSB(hue, saturation, brightness, validationContext);
  ...
  if (duration !== undefined && typeof duration !== 'number') {
    throw new TypeError('LIFX color zones method expects duration to be a number');
  }
  if (callback !== undefined && typeof callback !== 'function') {
    throw new TypeError('LIFX color zones method expects callback to be a function');
  }
